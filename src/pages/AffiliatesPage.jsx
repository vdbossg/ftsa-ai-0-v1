...
const handleAffiliateRegister = async (e) => {
  ...
  try {
    setLoading(true);
    const res = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/affiliate/register`, formData);
    setAffiliateData(res.data); // backend should return isRegistered:true + referralLink
  ...
  method: withdrawMethod,
    ...
    // Refresh affiliate data to update available balance
    const updated = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/affiliate/${user.id}`);
  ...
  return (
    <div className="affiliates-page" style={{ backgroundColor: "#000000", color: "#00FFFF", fontFamily: "Orbitron, sans-serif", minHeight: "100vh", padding: "2rem" }}>
      ...
      {/* Affiliate Registration - Hidden if user already registered */}
...
{affiliateData?.isRegistered && (
  <section className="withdrawal-panel neon-glow-border" style={{ marginTop: "2rem" }}>
    ...
    <p style={{ marginTop: "1rem", fontSize: "0.9rem", color: "#FF4500" }}>
      Note: You will receive a confirmation email. Verify to complete withdrawal.
    </p>
  </section>
)}
